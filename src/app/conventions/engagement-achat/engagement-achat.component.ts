...
@Component({
  selector: 'app-engagement-achat',
  templateUrl: './engagement-achat.component.html',
  styleUrls: ['./engagement-achat.component.scss']
})
export class EngagementAchatComponent implements OnInit {
  ...
  fetchAnnonceById(id: number): void {
    this.annonceService.getAnnonceById(id).subscribe(
      (data) => {
        ...
        this.router.navigate(['dashboard/engagmentById', id]);
      },
      (error) => {
  ...
  loadAnnonceData(page: number): void {
    this.annonceService.getAnnouncementsByType('engagementAchat', page).subscribe((data: any) => {
