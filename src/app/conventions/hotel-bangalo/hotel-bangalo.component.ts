...
@Component({
  selector: 'app-hotel-bangalo',
  templateUrl: './hotel-bangalo.component.html',
  styleUrls: ['./hotel-bangalo.component.scss']
})
export class HotelBangaloComponent {
  ...
  fetchAnnonceById(id: number): void {
    this.annonceService.getAnnonceById(id).subscribe(
      (data) => {
        ...
        this.router.navigate(['dashboard/annonceById', id]);
      },
      (error) => {
  ...
  loadAnnonceData(page: number): void {
    this.annonceService.getAnnouncementsByType('HotelBangalo', page).subscribe((data: any) => {
