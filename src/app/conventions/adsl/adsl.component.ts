...
@Component({
  ...
  styleUrls: ['./adsl.component.scss']
})
export class AdslComponent {
  ...
  fetchAnnonceById(id: number): void {
    this.annonceService.getAnnonceById(id).subscribe(
      (data) => {
        ...
        this.router.navigate(['dashboard/adslById', id]); //houni 2m1 bech ensadih el commponent 25er 
      },
      (error) => {
  ...
  loadAnnonceData(page: number): void {
    this.annonceService.getAnnouncementsByType('adsl', page).subscribe((data: any) => {
