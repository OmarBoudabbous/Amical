...
export class BonAchatComponent {
  ...
  fetchAnnonceById(id: number): void {
    this.annonceService.getAnnonceById(id).subscribe(
      (data) => {
        ...
        this.router.navigate(['dashboard/bonAchatById', id]);
      },
      (error) => {
  ...
  loadAnnonceData(page: number): void {
    this.annonceService.getAnnouncementsByType('bonAchat', page).subscribe((data: any) => {
